...
public class ZYProfilePresenter extends ZYListDataPresenter<ZYProfileContract.IView,ZYProfileModel,ZYAlbumDetail> implements ZYProfileContract.IPresenter{
    ...
    protected void loadData() {
        Observable<ZYResponse<ZYListResponse<ZYAlbumDetail>>> observable = Observable.zip(
                mModel.getUserInfo(userId), mModel.photos(userId), mModel.getAlbums(mPageIndex, mRows, Integer.parseInt(userId)), new Func3<ZYResponse<ZYUser>, ZYResponse<ZYListResponse<ZYPhoto>>, ZYResponse<ZYListResponse<ZYAlbumDetail>>, ZYResponse<ZYListResponse<ZYAlbumDetail>>>() {
                    @Override
